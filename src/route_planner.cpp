...
// - Return the pointer.
bool a_star_cond(RouteModel::Node *node_1,RouteModel::Node *node_2){

    if ( (node_1->h_value + node_1->g_value) < (node_2->h_value + node_2->g_value)){
        return false;
    }
    else {
        return true;
...
RouteModel::Node *RoutePlanner::NextNode() {
  ...
  std::sort (open_list.begin(), open_list.end(), a_star_cond);
...
std::vector<RouteModel::Node> RoutePlanner::ConstructFinalPath(RouteModel::Node *current_node) {
    ...
    //for (std::list<int>::reverse_iterator i = path_found.rbegin(); i != path_found.rend(); i++)
